...
from lovelace.models import Difficulty, User, Role, Problem, Tag, TestCase, SubmissonStatus, Submission
    ...
    for filename in folder.iterdir():
        if filename.suffix == ".toml":
            with open(folder / filename.name, "rb") as data:
                ...
                problem["testcases"] = [TestCase(**testcase) for testcase in problem["testcases"]]
...
def get_and_add_submissions(session):
    if session.query(Problem).count() == 0 and session.query(User).count() == 0:
        return 
    ...
    with open("scripts/data/submissions.toml", "rb") as file:
        ...
        for submission in data["submissions"]:
            ...
            problem_id = session.query(Problem.id).filter(Problem.name == submission["problem_name"]).first()[0]
            submission.pop('problem_name')
            user_id = session.query(User.id).filter(User.username == submission["username"]).first()[0]
            submission.pop("username")

            submissions.append(Submission(**submission, problem_id=problem_id, user_id=user_id))
...
def populate():
    ...
    with Session(engine) as session:
        ...
        for i, problem in enumerate(problems):
            ...
            problems[i].tags = session.query(Tag).filter(Tag.name.in_(tags_names)).all()
